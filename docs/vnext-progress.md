...
| 9 | IN PROGRESS | [Phase 9 principal identity](vnext-phase9-principal-identity.md); formal owner/project/network/device/service principals, durable generation-fenced rebind/revoke state, kernel-authenticated local UID resolution, exact Keryx + Nodescale + managed-projection remote identity, concurrent-principal isolation, and Run Capsule v3 exact principal references are implemented locally. Closure requires exact-head PR CI and resulting `main` CI. |
