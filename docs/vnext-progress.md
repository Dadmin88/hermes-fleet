...
| 8 | IN PROGRESS | [Phase 8 Run Capsule lifecycle](vnext-phase8-run-capsule-lifecycle.md) is re-audited against the updated master plan. Reconciliation now separates logical Recipe and ResolvedRecipe identity, binds compiler/provenance plus optional exact Workflow step identity, fails closed on legacy Capsule shapes, and recovers ambiguous pre-run body creation only by exact plan identity. Closure still requires exact-head PR CI and resulting `main` CI. |
| 8A | LOCKED | Workflow → Recipe compilation and first-run discovery begins only after Phase 8 is canonically closed; existing Workflow Library/editor state remains non-authoritative and execution-unavailable until that compiler/admission path is implemented. |
| 9 | LOCKED | Principal identity is blocked until Phase 8 and Phase 8A are canonically closed in order. |
