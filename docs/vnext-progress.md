...
| 8A | NEXT | Workflow → Recipe compilation and first-run discovery is the next implementation phase after Phase 8 canonical merge/main CI; existing Workflow Library/editor state remains non-authoritative and execution-unavailable until that compiler/admission path is implemented. |
