...
| 8A | IN PROGRESS | [Phase 8A Workflow → Recipe compilation and discovery](vnext-phase8a-workflow-recipe-compilation.md); Workflow v2 adds compile-only Recipe Steps, deterministic Candidate/Validated/Resolved Recipe forms, complete CPU/RAM/GPU/storage/runtime/security requirements, provenance/unknown states, exact cache invalidation, adaptive proposals, GPU placement, and low-authority disposable discovery. Closure requires exact-head PR CI and resulting `main` CI. |
| 9 | LOCKED | Principal identity remains blocked until Phase 8A is canonically closed with exact PR-head and resulting `main` CI evidence. |
