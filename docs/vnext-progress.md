...
| 7 | IN PROGRESS | Run-scoped Hermes execution overrides. Canonical Agent-fork PR #5 is merged and green on `Dadmin88/hermes-agent:main`; Fleet reconciliation now pins that exact fork SHA and verifies the installed runtime seam. Closure still requires green Fleet PR CI and resulting `main` CI. |
