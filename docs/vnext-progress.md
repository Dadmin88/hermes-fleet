...
| 10 | IN PROGRESS | [Phase 10 immutable RunAuthority](vnext-phase10-immutable-run-authority.md); one content-addressed authority document now binds exact principal/Agent/Recipe/destination/policy/capabilities/resources/isolation/grants and derives the existing authority-bound Capsule/network/filesystem/broker surfaces. Durable state separately fences replay, expiry, cancellation and revocation. Closure requires exact-head PR CI and resulting `main` CI. |
| 11 | LOCKED | Scoped persistent memory remains blocked until Phase 10 is canonically closed with exact PR-head and resulting `main` CI evidence. |
