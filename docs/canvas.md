...
Fleet preserves historical `fleet.workflow-editor.v1` documents and now authors compile-capable drafts as `fleet.workflow-editor.v2`. Clipboard data remains `fleet.workflow-clipboard.v1`. Fleet owns durable definitions in SQLite as immutable numbered revisions. Creates start at version 1; updates require the expected latest version and append a new revision when content changes. Soft deletion hides an active definition while retaining exact historical revisions, and a tombstoned identity cannot be recreated implicitly.

Workflow v2 adds one compile-only runtime marker: the built-in **Recipe Step** carries `runtime: recipe`. That marker means the backend may deterministically compile the node into a Candidate Recipe. It does **not** mean the editor can execute the step or grant authority. Every other Workflow block remains `runtime: unavailable`; historical v1 requires `runtime: unavailable` for every node. The production topology-only Machine descriptor still cannot be authored in Workflow Mode. Workflow metadata remains `executionAvailable: false`, and the editor/backend still define no direct run, scheduler, reservation, admission, or remote-action authority endpoint.

A Workflow is therefore the human-facing orchestration graph; Recipe Steps compile into backend-neutral disposable-environment specifications carrying CPU/RAM/GPU/storage/runtime/network/filesystem/tool/secret/host-operation **requirements**. Later admission and RunAuthority decide what is actually permitted.
