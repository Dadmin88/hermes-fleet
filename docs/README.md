...
- [Phase 7 run-scoped Hermes overrides](vnext-phase7-run-scoped-overrides.md) - exact ContextVar-backed `fleet_runtime` binding, run-scoped toolset/iteration narrowing, attach-only container selection, live Docker proof, capability advertisement, and Fleet fail-closed support gating.
- [Phase 8 Run Capsule lifecycle](vnext-phase8-run-capsule-lifecycle.md) - durable temporary Capsule spec/store, generation-CAS recovery, exact plan/container discovery, duplicate-submission crash fence, quiescence/evidence/artifact/learning/revocation ordering, idempotent cleanup, and persistent-Agent real-Docker proof.
